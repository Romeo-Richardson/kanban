...
const Sidenav = ({ boards }: props): React.ReactNode => {
  ...
  return (
    <div className=" m-h-screen w-64 relative flex flex-col border-r-[1px] border-solid border-gray-800">
      ...
      <div className="mw-full h-14 mb-2">
        <div
          className="w-[90%] h-full duration-300 bg-green-600 hover:cursor-pointer hover:bg-green-700 flex items-center rounded-tr-[2rem] rounded-br-[2rem]"
          onClick={() => {
            setBoardModal(true);
          }}
        >
      ...
      <div className="mw-full overflow-auto grow  relative">
        <div className="absolute w-full top-0 flex flex-col gap-2">
          {boards.map((item, key) => {
            return (
              <div
                className={`w-[90%] h-14 ${
